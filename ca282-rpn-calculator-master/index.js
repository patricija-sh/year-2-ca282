...
// Code taken from get request.

app.get("/length", function(req, res)
{
    response = stack.length;
    res.send(JSON.stringify(response));
})

// Implementation of push.
// Code taken from post request.

app.post("/push", function(req, res)
{
    if (req.body && req.body.hasOwnProperty("values") && Array.isArray(req.body.values))
    {
        stack = stack.concat(req.body.values);
        console.log(stack);
        console.log("pushing to stack", req.body.values);
        res.send("server... ok, pushing: $(req.body.values)\\n");
    }
    else
    {
        res.status(400).send("bad request (invalid push request)\\n");
    }
});
...
app.get("/add", function(req, res)
{
    ...
    {
        res.status(400).send("insufficient arguments for add\\n");
    }
});
...
app.get("/subtract", function(req, res)
{
    ...
    {
        res.status(400).send("insufficient arguments for subtract\\n");
    }
});
...
app.get("/multiply", function(req, res)
{
    ...
    {
        res.status(400).send("insufficient arguments for multiply\\n");
    }
});
...
app.get("/divide", function(req, res)
{
    ...
    {
        res.status(400).send("insufficient arguments for divide\\n");
    }
});
